...
def make_env(env_id, env_conf):
    environment = gym.make(env_id, render_mode=env_conf["render_mode"])
    if "NoFrameskip" in env_id:
        assert "NoFrameskip" in environment.spec.id
        environment = NoopResetEnv(environment, noop_max= 30)
        environment = MaxAndSkipEnv(environment, skip = env_conf["skip_rate"])
    ...
    environment = AtariRescale(environment, env_conf["useful_region"])
    
    if env_conf["normalize_observation"]:
        environment = NormalizedEnv(environment)
        
    environment = FrameStack(environment, env_conf["num_frames_to_stack"])

    if env_conf["clip_reward"]:
        environment = ClipReward(environment)
...
class NoopResetEnv(gym.Wrapper):
    ...
    def reset(self):
        ...
        for _ in range(noops):
            observation, _, done, _, _ = self.env.step(self.noop_action)
        return observation
